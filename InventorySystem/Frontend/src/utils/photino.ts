...
/**
 * Opens the native OS "Save File" dialog filtered to specific files.
 * Returns the chosen path, or null if cancelled.
 * Falls back to null immediately if not running inside Photino.
 */
export function pickSaveFile(fileType?: string, defaultPath?: string): Promise<string | null> {
  if (!isPhotino()) return Promise.resolve(null);
  ...
  return new Promise((resolve) => {
    ...
    pending.set(requestId, (msg) => resolve(msg.path ?? null));
    photinoExternal!.sendMessage(
      JSON.stringify({ type: 'pick-file', mode: 'save', requestId, fileType, defaultPath })
    );
  });
}

/**
 * Sends base64 file data to C# to write to disk.
 */
export function saveFileData(path: string, base64Data: string): void {
  if (!isPhotino()) return;
  photinoExternal!.sendMessage(
    JSON.stringify({ type: 'save-file-data', path, base64Data })
  );
}
