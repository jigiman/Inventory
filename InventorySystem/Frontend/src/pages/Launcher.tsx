...
export default function Launcher({ recentDatabases, onReady, theme, onThemeChange }: LauncherProps) {
  ...
  const [showOpenOther, setShowOpenOther] = useState(false);
                    ...
                    !showOpenOther ? (
                      ...
                      <div className="space-y-3">
                        <Input
                          ...
                          placeholder="/path/to/inventory.db"
                          value={openPath}
                          onChange={e => setOpenPath(e.target.value)}
                          onKeyDown={e => e.key === 'Enter' && handleOpen(openPath)}
                        />
                        <div className="flex gap-2">
                          <Button variant="outline" size="sm" className="flex-1" onClick={() => { setShowOpenOther(false); setOpenPath(''); setError(''); }}>
                            Cancel
                          </Button>
                          <Button size="sm" className="flex-1" onClick={() => handleOpen(openPath)} disabled={loading || !openPath.trim()}>
                            {loading ? <Loader2 size={14} className="animate-spin" /> : 'Open'}
                          </Button>
                        </div>
                      </div>
                    )
